...
export function ComposeEditor({ isReply, initialData, replyOriginalBody, replyOriginalSender, onFieldChange, onCancel, onSaveDraft, onSend, onAttachFile, attachments, onRemoveAttachment }: ComposeEditorProps): ReactElement {
    ...
    const handleAddToken = (field: "to" | "cc", value: string) => {
        const trimmed = value.trim();
        if (!trimmed) return;

        if (field === "to") {
            const newTokens = [...toTokens, trimmed];
            setToTokens(newTokens);
            setToInput("");
            onFieldChange("to", newTokens.join(","));
        } else {
            const newTokens = [...ccTokens, trimmed];
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, field: "to" | "cc", inputVal: string) => {
        if (e.key === "Enter" || e.key === ",") {
            e.preventDefault();
            handleAddToken(field, inputVal);
        } else if (e.key === "Backspace" && !inputVal) {
            handleRemoveToken(field, field === "to" ? toTokens.length - 1 : ccTokens.length - 1);
        }
    };
            ...
            <div className="compose-form">
                <div className="form-group">
                    ...
                    <div className="token-input">
                        <div className="token-list">
                            {toTokens.map((rec, i) => (
                                <span key={i} className="token">{rec} <button className="remove-token" onClick={() => handleRemoveToken("to", i)}>×</button></span>
                            ))}
                            <input
                                ...
                                onBlur={() => handleAddToken("to", toInput)}
                            />
                        </div>
                ...
                <div className="form-group">
                    ...
                    <div className="token-input">
                        <div className="token-list">
                            {ccTokens.map((rec, i) => (
                                <span key={i} className="token">{rec} <button className="remove-token" onClick={() => handleRemoveToken("cc", i)}>×</button></span>
                            ))}
                            <input
                                className="token-text-input"
                                placeholder=""
                                ...
                                onBlur={() => handleAddToken("cc", ccInput)}
                            />
                        </div>
                        <span className="token-expand">&#x25BE;</span>
                    </div>
